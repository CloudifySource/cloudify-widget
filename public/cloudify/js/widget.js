  ...
  var origin_page_host = origin_page_url.replace( /([^:]+:\/\/[^\/]+).*/, '$1' );
    ...
    function handleUpdateStatusSuccess( data )
    {
        ...
        //    data.status.output.push(ellipsis.substring( 0, (ellipsis_iteration++ % 5) + 1) );
            data.status.output.splice(data.status.output.length - 1, +(data.status.output[data.status.output.length - 1].indexOf('.') == 0), ellipsis.substring( 0, (ellipsis_iteration++ % 5) + 1));
