...
public class WidgetServer
{
	...
	public WidgetInstance deploy( String apiKey )
	{
		...
		ServerNode server = ApplicationContext.getServerPool().get();
		if ( server == null )
			throw new ServerException(ResMessages.getString("no_available_servers"));
		
		widget.countLaunch();
		
		String instanceId = ApplicationContext.getDeployManager().fork( server, recipeDir).getId();
	...
	public void undeploy( String instanceId )
	{
		...
		ApplicationContext.getServerPool().destroy(instanceId);
	}

	
	public Status getWidgetStatus( String instanceId )
	{
		DeployManager.ProcExecutor pe = ApplicationContext.getDeployManager().getExecutor( instanceId );
		if ( pe == null )
