 ...
 * It also provides ability to startup with a cold init which deletes a running server. See {@link Config} class.
 * 
...
public class ServerPool
{
	...
	private void init()
	{
		...
		List<Server> serverList = ApplicationContext.getServerBootstrapper().getServerList();
			...
			else
			{
				if ( server.isBusy() )
				{
				   ...
				   if ( server.isTimeLimited() )
				     ApplicationContext.getExpiredServersCollector().scheduleToDestroy(server);
				}
				else
		...
		if ( serverList.size() < SERVER_POOL_MIN_NODES )
		{
			...
			List<ServerNode> servers = ApplicationContext.getServerBootstrapper().createServers(serversToInit);
